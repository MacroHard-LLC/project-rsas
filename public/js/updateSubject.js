...
editSubjectModal.addEventListener('show.bs.modal', function() {
    ...
    ['input','change'].forEach(evt =>
        updateSubjectForm.querySelectorAll(".form-control, .form-select, .form-check-input").forEach(input => {
            input.addEventListener(evt, updateSubjectInputListener(input));
        })
    );
});

const updateSubjectInputListener = (input) => {
    return () => {
        $("#" + input.getAttribute("name") + "_error").children("span").text("");
...
editSubjectModal.addEventListener('hidden.bs.modal', function(){
    ...
    const inputElements = editSubjectModal.querySelectorAll('.form-control, .form-select, .form-check-input');
    inputElements.forEach(input => {
        input.removeEventListener('input', updateSubjectInputListener);
        input.removeEventListener('change', updateSubjectInputListener);
    });
...
function showUpdateSubjectClientError(input){
    ...
    } else if (input_name = "Usubject_name" && input.validity.patternMismatch){
